...
// Copyright (c) 2017-2018 The Kingscoin developers
...
#if defined(NDEBUG)
#error "Kingscoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in ukgs) are considered zero fee (for relaying and mining)
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight <= Params().LAST_POW_BLOCK())
        nSubsidy = 6000 * COIN;
    else if (nHeight <= 50000 && nHeight > Params().LAST_POW_BLOCK())
        nSubsidy = 12 * COIN;
...
{
    RenameThread("kingscoin-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    CAmount nExpectedMint = GetBlockValue(pindex->pprev->nHeight + 1 || pindex->pprev->nHeight);
    if (block.IsProofOfWork())
        nExpectedMint += nFees;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // KGS: We use certain sporks during IBD, so check to see if they are
